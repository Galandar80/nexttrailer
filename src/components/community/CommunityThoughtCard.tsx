    ...
    <div className="bg-secondary/10 rounded-lg p-4 border border-white/5 space-y-2">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <Link to={`/profilo/${thought.userId}`} className="font-semibold hover:text-accent">
              {thought.userName}
            </Link>
      ...
      {thought.sharedFromId && (
        <div className="bg-secondary/20 rounded-md p-3 space-y-1">
          ...
          <Link
            to={`/profilo/${thought.sharedFromUserId}`}
          ...
          {thought.sharedFromTargetTitle && thought.sharedFromTargetType && thought.sharedFromTargetId ? (
            thought.sharedFromTargetType === "person" ? (
              <Link to={`/person/${thought.sharedFromTargetId}`} className="text-sm text-accent hover:underline">
                {thought.sharedFromTargetTitle}
              </Link>
            ) : (
              <Link to={`/${thought.sharedFromTargetType}/${thought.sharedFromTargetId}`} className="text-sm text-accent hover:underline">
                {thought.sharedFromTargetTitle}
              </Link>
            )
          ) : null}
      ...
      {thought.targetTitle && thought.targetType && thought.targetId ? (
        thought.targetType === "person" ? (
          <Link to={`/person/${thought.targetId}`} className="text-sm text-accent hover:underline">
            {thought.targetTitle}
          </Link>
        ) : (
          <Link to={`/${thought.targetType}/${thought.targetId}`} className="text-sm text-accent hover:underline">
            {thought.targetTitle}
          </Link>
        )
      ) : null}
