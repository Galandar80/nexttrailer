...
const Navbar = () => {
  ...
  const handleGoogleLogin = async () => {
    if (!isFirebaseEnabled) {
      toast({ title: "Accesso con Google non disponibile", description: "Firebase non configurato", variant: "destructive" });
      return;
    }
    setAuthLoading(true);
    try {
    ...
    <nav className="py-4 px-4 md:px-8 border-b border-muted/30 z-50 relative bg-background/80 backdrop-blur-md sticky top-0">
      <div className="max-w-screen-2xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {searchResults.map((item) => {
                      ...
                      return (
                        ...
                        >
                          <div className="h-12 w-9 rounded overflow-hidden bg-secondary/40 flex items-center justify-center text-xs text-muted-foreground">
                            {imageUrl ? (
                              <OptimizedImage
                                src={imageUrl}
          ...
          <Button variant="ghost" className="md:hidden" size="icon" onClick={() => router.push('/search')}>
            <Search className="h-4 w-4" />
          </Button>
          {canAccess && (
            <Link href="/notifiche" className="hidden md:inline-flex">
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent side="right" className="w-72 sm:w-80">
              ...
              <div className="mt-6 flex flex-col gap-2">
                ...
                <Button variant="ghost" className="justify-start" asChild>
                  <Link href="/catalogo" onClick={() => setIsMobileMenuOpen(false)}>Catalogo</Link>
                </Button>
                <Button variant="ghost" className="justify-start pl-8" asChild>
                  <Link href="/oscar" onClick={() => setIsMobileMenuOpen(false)}>Oscar</Link>
                </Button>
                <Button variant="ghost" className="justify-start pl-8" asChild>
                  <Link href="/genres" onClick={() => setIsMobileMenuOpen(false)}>Sfoglia</Link>
                </Button>
                <Button variant="ghost" className="justify-start" asChild>
                  <Link href="/search" onClick={() => setIsMobileMenuOpen(false)}>Cerca</Link>
                </Button>
                {canAccess && (
                  <>
                    <Button variant="ghost" className="justify-between" asChild>
                      <Link href="/notifiche" onClick={() => setIsMobileMenuOpen(false)}>
                        <span>Notifiche</span>
                        {unreadNotifications > 0 && (
                    ...
                    <Button variant="ghost" className="justify-between" asChild>
                      <Link href="/storico" onClick={() => setIsMobileMenuOpen(false)}>
                        <span>Storico</span>
                        {libraryCount > 0 && (
                          <span className="bg-accent text-white text-[10px] rounded-full h-4 min-w-4 px-1 flex items-center justify-center">
                            {libraryCount}
                    ...
                    <Button variant="ghost" className="justify-start" asChild>
                      <Link href="/profilo" onClick={() => setIsMobileMenuOpen(false)}>Profilo</Link>
                    </Button>
                    <Button variant="ghost" className="justify-start" asChild>
                      <Link href="/preferenze" onClick={() => setIsMobileMenuOpen(false)}>Preferenze</Link>
                    </Button>
                  </>
                )}
                {!user && (
              ...
              <DropdownMenuContent className="w-56" align="end" forceMount>
                ...
                {canAccess ? (
                  <>
                    ...
                    <DropdownMenuItem onClick={() => router.push('/notifiche')}>
                      Notifiche
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => router.push('/watchlist')}>
                      Watchlist
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => router.push('/storico')}>
                      Storico
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => router.push('/preferenze')}>
                      Preferenze
                    </DropdownMenuItem>
                  </>
                ) : (
                    ...
                    <TabsContent value="login">
                      <form onSubmit={handleEmailLogin} className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Button type="button" variant="outline" className="w-full" onClick={handleGoogleLogin} disabled={authLoading || !isFirebaseEnabled}>
                            Accedi con Google
                          </Button>
                        </div>
                      </form>
