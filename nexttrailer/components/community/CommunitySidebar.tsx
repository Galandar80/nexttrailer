...
const CommunitySidebar = ({
  ...
  return (
    <div className="space-y-4">
            ...
            {userSearchResults.map((result) => (
              <div key={result.id} className="bg-secondary/10 rounded-lg p-3 border border-white/5 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Link href={`/profilo/${result.id}`} className="font-semibold hover:text-accent">
                      {result.displayName}
                    </Link>
                    {result.bio && (
      ...
      {userId && (
        <Link href={`/profilo/${userId}/seguiti`} className="text-sm text-accent hover:underline">
          Gestisci i seguiti
        </Link>
      )}
      ...
      {suggestions.map((suggestion) => (
        <div key={suggestion.id} className="bg-secondary/10 rounded-lg p-4 border border-white/5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <Link href={`/profilo/${suggestion.id}`} className="font-semibold hover:text-accent">
