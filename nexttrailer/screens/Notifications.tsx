    ...
    if (item.type === "follow") {
      return (
        <Button variant="outline" size="sm" asChild>
          <Link href={`/profilo/${item.actorId}`}>Vedi profilo</Link>
        </Button>
      );
    }
    return (
      <Button variant="outline" size="sm" asChild>
        <Link href="/community">Apri community</Link>
      </Button>
    );
  };
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 space-y-1">
            <p className="text-sm">
              <Link href={`/profilo/${item.actorId}`} className="font-semibold hover:text-accent">
                {item.actorName}
              </Link>{" "}
