...
const Library = () => {
          ...
          await Promise.all(
            seasonEntries.map(async ([seasonKey, watchedEpisodes]) => {
              ...
              if (!episodes) {
                ...
                const seasonData = await seasonResponse.json();
                const fetchedEpisodes = Array.isArray(seasonData.episodes) ? seasonData.episodes : [];
                cache.set(cacheKey, fetchedEpisodes);
  ...
  const clampedVisibleCount = useMemo(
    () => Math.max(PAGE_SIZE, Math.min(visibleCount, filteredItems.length)),
    [visibleCount, filteredItems.length]
  );
  const visibleItems = useMemo(
    () => filteredItems.slice(0, clampedVisibleCount),
    [filteredItems, clampedVisibleCount]
  );
      ...
      <main className="max-w-screen-xl mx-auto px-4 md:px-8 py-8">
        ...
        {items.length === 0 ? (
          ...
          <>
                  ...
                  {eventsForSelected.map((event) => (
                    ...
                    >
                      ...
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/${event.media_type}/${event.id}`}>Dettagli</Link>
                      </Button>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              {Object.entries(statusLabels).map(([key, label]) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    setStatusFilter(key as LibraryStatus);
                    setVisibleCount(PAGE_SIZE);
                  }}
                >
                  {label}
            ...
            {filteredItems.length > clampedVisibleCount && (
