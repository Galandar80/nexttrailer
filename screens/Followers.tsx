...
const FollowersPage = () => {
  ...
  useEffect(() => {
    ...
    const loadFollowers = async () => {
      ...
      try {
        ...
        const followerIds = followersSnapshot.docs.map((entry) => entry.data()?.followerId).filter(Boolean) as string[];
        const userProfiles = await Promise.all(followerIds.map(async (followerId) => {
        ...
        if (canAccess && user) {
          ...
          const followingIds = followingSnapshot.docs.map((entry) => entry.data()?.followedId).filter(Boolean) as string[];
          setFollowedIds(new Set(followingIds));
        } else {
          setFollowedIds(new Set());
        }
      } catch {
        setError("Impossibile caricare i follower");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <SEO title="Follower" description="Elenco dei follower" />
      <Navbar />
      <main className="max-w-screen-lg mx-auto px-4 py-8 space-y-6">
        <div>
          <h1 className="text-3xl font-bold">Follower</h1>
          {profileId && (
            <Link href={`/profilo/${profileId}`} className="text-sm text-accent hover:underline">
              Torna al profilo
            </Link>
          )}
        </div>
        ...
        {!isLoading && !error && profiles.length === 0 && (
          <div className="text-muted-foreground">Nessun follower disponibile.</div>
        )}
            ...
            {profiles.map((profile) => (
              <div key={profile.id} className="bg-secondary/10 rounded-lg p-4 border border-white/5 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Link href={`/profilo/${profile.id}`} className="font-semibold hover:text-accent">
                      {profile.displayName}
                    </Link>
